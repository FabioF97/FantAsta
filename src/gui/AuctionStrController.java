...
/**
 * This class is the AuctionStr controller and is responsible 
...
public class AuctionStrController {
	...
	public ObservableList<Player> getPlayers() throws SQLException{
		...
		if(sceneSelector == false) {
			list = db.getStr1(championship.getName());
		}
		else {
			list = db.getStr2(championship.getName());
		}
		for(Player p : list) {
	 ...
	 * update the player list on the database and save users
